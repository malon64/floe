...
/// - `execution.runner.type` is one of the recognized values (`local`, `kubernetes`, `kubernetes_job`) when present.
...
fn validate_runner_type(runner_type: &str) -> FloeResult<()> {
    const KNOWN_RUNNERS: &[&str] = &["local", "kubernetes", "kubernetes_job"];
