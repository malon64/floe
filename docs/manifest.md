...
A manifest is generated by one runner and replayed by a **different** one, so every local
path it records must be **absolute** — a relative path would resolve against the wrong base at
replay time. The absolute base depends on the runtime the manifest targets, selected with
`--runtime image|cli` (recorded as `runtime_env` in the manifest). When the flag is omitted,
Floe auto-detects: a container (a `/work` working dir or `/.dockerenv`) picks `image`,
otherwise `cli`. Override detection with the `FLOE_RUNTIME` environment variable
(`image` / `cli`).

- **`image`** — local paths are absolutized under the container work-root: `-c domains/orders.yml`
  is stored as `local:///work/domains/orders.yml` (recorded `work_root: /work`, overridable
  with `FLOE_WORK_ROOT`). Because every container uses the same `/work` mount, this is **both**
  portable for remote replay **and** reproducible across container runs — so `manifest_id`
  (a hash of `config_uri` + `config_checksum`) is byte-stable in CI. Use this whenever the
  manifest is deployed to object storage and replayed by a remote or containerized runner.
- **`cli`** — local paths are recorded **host-absolute**: `-c domains/orders.yml` is
  canonicalized against the generating host's working directory and stored as, e.g.,
  `local:///Users/you/repo/domains/orders.yml`. This is resolvable on that host but is
  intentionally **not** reproducible across machines — for a stable, committed, CI-diffable
  `manifest_id`, generate with `--runtime image`.

Remote config URIs (`s3://`, `gs://`, `abfs://`) and already-absolute local paths are recorded
unchanged in both modes.

> **Changed in v0.6.7.** v0.6.6 briefly recorded local `config_uri` / `profile_uri` **relative**
> (`local://domains/orders.yml`) to make `manifest_id` reproducible across checkouts. That broke
> container/remote replay (the manifest resolved against its own bucket instead of `/work`;
> issue #443), so local URIs are absolute again. Cross-environment reproducibility now comes
> from `--runtime image` (fixed `/work`), not from relative paths.
...
> For **local** storage under `--runtime cli`, those fields resolve to host-absolute
> `local://<checkout>/...` URIs, so `manifest_revision` (like the host-absolute `config_uri`)
> differs between machines. Generate with `--runtime image` (fixed `/work`) or use remote
> storage for a stable, cross-environment revision.
