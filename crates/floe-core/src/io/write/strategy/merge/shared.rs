...
// Merge-key / scd2 / schema-evolution helpers live in the non-gated `keys`
// module so the DuckDB merge path and the run pipeline can reuse them when the
// `delta` feature is off. Re-export them here so the Delta merge code keeps
// referring to them as `shared::…`.
pub(crate) use super::keys::{
    default_schema_evolution_summary, resolve_merge_column_mappings, resolve_merge_ignore_columns,
    resolve_merge_key, resolve_scd2_system_columns, Scd2SystemColumns,
};
...
pub(crate) fn write_standard_delta_version_with_perf(
    ...
    let batch = crate::io::write::arrow_convert::dataframe_to_record_batch(df, entity)?;
...
pub(crate) fn source_record_batch(
    ...
    crate::io::write::arrow_convert::dataframe_to_record_batch(source_df, entity)
}
