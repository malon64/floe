    ...
    match target {
        ...
        Target::S3 {
            ...
            let key = match format {
                "parquet" => io::storage::s3::build_parquet_key(base_key, source_stem),
                "delta" | "iceberg" => {
...
pub(super) fn write_rejected_output(
    ...
    match target {
        ...
        Target::S3 {
            ...
            let key = match format {
                "csv" => io::storage::s3::build_rejected_csv_key(base_key, source_stem),
                _ => {
                    return Err(Box::new(ConfigError(format!(
...
pub(super) fn write_rejected_raw_output(
    ...
    match target {
        ...
        Target::S3 {
            ...
            let key = io::storage::s3::build_rejected_raw_key(base_key, &input_file.source_name);
            let client = cloud.client_for(resolver, storage, entity)?;
...
pub(super) fn write_error_report_output(
    ...
    match target {
        ...
        Target::S3 {
            ...
            let key = io::storage::s3::build_reject_errors_key(base_key, source_stem);
            let client = cloud.client_for(resolver, storage, entity)?;
