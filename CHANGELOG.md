...
- **Soft-buffered accepted writes — cap per-entity memory at `max_size_per_file`** (addresses #332):
  ...
  - **Behavioural note (Parquet only, non-transactional sinks)**: because flushes commit synchronously while the file loop is still iterating, a failure on a *later* file in the same batch (read / validation / rejected-write / archive) can leave *earlier* files' accepted rows already on disk — and in `overwrite` mode the first flush may already have replaced the previous dataset. The run report and incremental state are not committed in that case, so the same inputs are re-processed on the next run; downstream consumers reading the accepted directory directly may temporarily see the partial state. Each file's archive call always runs before its rows enter the buffer, so a file's data only reaches the sink after its archive has succeeded. Delta and Iceberg sinks remain transactional per commit and never expose torn state. Documented under `sink.accepted.options.max_size_per_file` in `docs/config.md`.
